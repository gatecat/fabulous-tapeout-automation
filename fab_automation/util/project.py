...
__all__ = ["TileType", "PinConfig", "FabricConfig", "Project"]
...
@dataclass
class Project:
    tiles: dict[str, TileType] = field(default_factory=dict)
    pin_cfg: Optional[PinConfig] = None
    fab_cfg: Optional[PinConfig] = None
    ...
    def parse(filename):
        ...
        while lines:
            ...
            cmd = sl[0].lower()
            if cmd == "tile":
                tile = _parse_section(TileType, lines, init_args=dict(tiletype=sl[1]))
...
if __name__ == '__main__':
    import sys, pprint
    pprint.pprint(Project.parse(sys.argv[1]))